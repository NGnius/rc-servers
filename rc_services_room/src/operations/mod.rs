...
pub fn handler(init_ctx: &crate::InitConfig) -> OperationsHandler<crate::UserTy> {
    OperationsHandler::new()
        ...
        .add(crf_vehicle_data::crf_item_data_provider(&init_ctx.factory, init_ctx.parsers.converter()))
        .add(crf_purchase::crf_copy_to_bay_provider(&init_ctx.factory, init_ctx.parsers.weapon_order(), init_ctx.parsers.cpu_counter(), init_ctx.parsers.converter()))
